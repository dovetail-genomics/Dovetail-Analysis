...
Now that you have a genome file, index file and a reference fasta file you are all set to align your LinkPrep library to the reference. Please note the specific settings that are needed to map mates independently and for optimal results with our proximity library reads.
...
The ``pairtools split`` command is used to split the final ``.pairsam`` into two files: ``.sam`` (or ``.bam``) and ``.pairs`` (``.pairsam`` has two extra columns containing the alignments from which the LinkPrep pair was extracted, these two columns are not included in ``.pairs`` files)
